...
The \ensuremath{\Conid{Temporal}} moand is ``weak'', in the sense that the standard monad
laws do not hold in general.  For example, one of the unit laws is that:
%%
\begin{equation}
%|(return x) >>= (\y -> f y)| \equiv |f x|
\ensuremath{\Varid{m}\bind \Varid{return}} \equiv \ensuremath{\Varid{m}}
\label{law-example}
\end{equation}
%%
where \ensuremath{\Varid{m}\mathbin{::}\Conid{Temporal}\;\Varid{a}}.  However, for \ensuremath{\Conid{Temporal}} this law is
violated in cases where \ensuremath{\Varid{m}} depends on the current time. For example, let
\ensuremath{\Varid{m}} be defined:
%
\begin{hscode}\SaveRestoreHook
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{10}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{m}\mathrel{=}\mathbf{do}\;{}\<[10]%
\>[10]{}\Varid{kernelSleep}\;\mathrm{1}{}\<[E]%
\\
\>[10]{}\Varid{start}\leftarrow \Varid{start}{}\<[E]%
\\
\>[10]{}\Varid{end}\leftarrow \Varid{time}{}\<[E]%
\\
\>[10]{}\Varid{return}\;(\Varid{diffTime}\;\Varid{end}\;\Varid{start})\mbox{\onelinecomment  duration of computation}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
%%
...
\column{B}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\mathbin{*}\Conid{Main}\mathbin{>}\Varid{runTime}\mathbin{\$}\Varid{m}\bind \Varid{return}{}\<[E]%
\\
\>[B]{}\mathrm{1.001113}\;\Varid{s}{}\<[E]%
\\
\>[B]{}\mathbin{*}\Conid{Main}\mathbin{>}\Varid{runTime}\mathbin{\$}\Varid{m}{}\<[E]%
\\
\>[B]{}\mathrm{1.00114}\;\Varid{s}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
(note, these results are also non-deterministic).  The difference in
results follows from the additional reduction required on \ensuremath{(\bind )} in
...
\ensuremath{\Varid{time}} would be disingenuous, since we are evaluating \ensuremath{\Varid{m}\bind \Varid{return}}
and \ensuremath{\Varid{m}} at different start times.
...
\\
\>[B]{}\hsindent{5}{}\<[5]%
\>[5]{}\Varid{return}\;\Varid{a}\mathrel{=}\Conid{TE}\;(\lambda \anonymous \to \Varid{return}\;(\Varid{a},[\mskip1.5mu \mskip1.5mu])){}\<[E]%
\\
\>[B]{}\hsindent{5}{}\<[5]%
